...
namespace Demo1
{
    public class SinglePlayer : Game
    {
        ...
        public SinglePlayer(GameScene scene, int width, int height)
        {
        ...
        {
            _ai = ai;
            _ai.SetParameters(_mallet2Polygon, _puckPolygon, _goal2, true);
        }
        ...
        public override void Tick()
        {
            ...
            if (_ai != null)
            {
                var max = _centerLineY;
                if (_ai.TopSide && _puck.Position.Y > _centerLineY + 10) max = _centerLineY / 4;
                if (!_ai.TopSide && _puck.Position.Y < _centerLineY - 10) max = _areaHeight - _centerLineY / 4;
                _ai.Tick(_mallet2.Position, max, _centerLineY, _puck.Size.X, _areaWidth - _puck.Size.X);
                foreach (var p in _collisionPlane.TickFreeMovement(_mallet2Polygon))
                {
                    if (p == _puckPolygon)
                    {
                        playStickHit();
                ...
                _mallet2Polygon.Offset(limitMallet(_mallet2.Position, _mallet2.Size, true));
            }
            base.Tick();
        }
    }
